...
def convert_wav(wav_file):
    # Generate the temporary WAV file name based on the input WAV file name
    temp_wav_file = wav_file.replace('.wav', '__TEMP.wav')

    print(f"Converting WAV file {wav_file} to {temp_wav_file}...")
    ffmpeg_command = [
        "ffmpeg",
        "-i", wav_file,
    ...
    print(f"\nConversion of {wav_file} complete.")
    return temp_wav_file  # Return the generated temporary WAV file name
...
def process_wav_srt_pair(wav_file, srt_file, output_dir):
    ...
    # Generate the temporary WAV file name
    temp_wav_file = convert_wav(wav_file)
...
def process_all_files_in_bulk():
    # Create a multiprocessing pool for WAV conversion
    conversion_pool = multiprocessing.Pool(processes=4)  # Limit to 4 processes at a time

    # Create a list to store the paths of all the output directories
    output_dirs = []

    # Process all WAV and SRT files in the current directory
        ...
        for filename in filenames:
            if filename.endswith('.wav'):
                ...
                if os.path.exists(srt_file):
                    ...
                    # Generate a temporary WAV file for each input WAV file
                    temp_wav_file = convert_wav(wav_file)

    # Close the WAV conversion pool and wait for all conversions to complete
    conversion_pool.close()
    conversion_pool.join()

    # Create a multiprocessing pool for segment processing
    processing_pool = multiprocessing.Pool(processes=4)  # Limit to 4 processes at a time

    # Process WAV conversions and segment processing
    for dirpath, dirnames, filenames in os.walk('.'):
        for filename in filenames:
            if filename.endswith('.wav'):
                wav_file = os.path.join(dirpath, filename)
                ...
                if os.path.exists(srt_file):
                    ...
                    # Enqueue the segment processing task with the generated temporary WAV file name
                    processing_pool.apply_async(process_wav_srt_pair, args=(wav_file, srt_file, output_dir))

    # Close the segment processing pool and wait for all processes to complete
